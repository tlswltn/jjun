...
public class BoardLikeController implements Controller {
    ...
    public ModelAndView execute(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            // 좋아요 등록
            BoardService.getInstance().insertBoardLike(bno, id); // 서비스 계층을 통해 좋아요 추가
            json.put("msg", "해당 게시글에 좋아요를 하셨습니다."); // 성공 메시지 저장
        } catch (Exception e) {
            // 예외 발생 시(이미 좋아요 등록된 경우), 좋아요를 취소하는 로직 처리
            BoardService.getInstance().deleteBoardLike(bno, id); // 서비스 계층을 통해 좋아요 삭제
            json.put("msg", "해당 게시글에 좋아요를 취소 하셨습니다."); // 취소 메시지 저장
        }
